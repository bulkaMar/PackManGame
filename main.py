...
class Ghost:
    ...
    def move_pinky(self):
        """
    Керує рухом привида Pinky залежно від його поточного напрямку та цілей на карті гри.

    Що робить:
    Функція визначає напрямок руху привида Pinky залежно від цілей та перешкод на мапі гри.
    Якщо Pinky може рухатися прямо, він рухатиметься у поточному напрямку.
    Якщо Pinky зіштовхується з перешкодою, він обиратиме інший доступний напрямок.

    Що повертає:
    Повертає кортеж з трьома елементами:
    - Нове значення `x_pos`, яке відповідає новому положенню привида Pinky по осі X.
    - Нове значення `y_pos`, яке відповідає новому положенню привида Pinky по осі Y.
    - Нове значення `direction`, яке відповідає новому напрямку руху привида Pinky.
    """
        if self.direction == 0:
            if self.target[0] > self.x_pos and self.turns[0]:
                self.x_pos += self.speed
